...
from model.metric import top_k_accuracy
...
def main():
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in pbar:
            ...
            acc1, acc5 = top_k_accuracy(pred_logits, target, topk=(1, 5))
    ...
    print("Evaluation of model {:s} on dataset {:s}, Acc@1: {:.4f}, Acc@5: {:.4f}".format(config.model_arch,
                                                                                          config.dataset,
                                                                                          np.mean(acc1s),
                                                                                          np.mean(acc5s)))
